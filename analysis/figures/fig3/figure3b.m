...
%     [coefEsts,R,J,CovB,MSE,ErrorModelInfo] = nlinfit(myt(cutoff:end-1), sl(cutoff:end), modelFun, [1 scale]);
    ...
    term = cutoff+find(sl2(cutoff:end)>0.6,1);
    if(isempty(term))
        term = length(sl);
    end
    if(allp(ind,11)<0&&~isempty(spt)&&term-spt>10&&allp(ind,6)>0.01)
...
%         allG = [allG sl2(min(sl)==sl)];
        plot(myt(spt:term),sl2(spt:term),'.','DisplayName',[num2str(allp(ind,2)) ' ' num2str(abs(allp(ind,3))) ' ' num2str(allp(ind,5)) ' ' num2str(allp(ind,6)) ' ' num2str(abs(allp(ind,11)/allp(ind,6))) ' ' num2str(sl2(end)) ' ' num2str(abs(allp(ind,11))./mean(allg(ind,spt:end),2)./eta)])
        hold on
    else
        go=[go; 0];
    end
