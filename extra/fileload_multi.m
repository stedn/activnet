...
origbp = pwd;
%#ok<*ST2NM>
bp = '/Users/wmcfadden/xlrelax_weak/';
cd(bp);
files = dir;
files = {files.name};
stoG = [];
...
%             dy = yt-repmat(yt(1,:),size(yt,1),1);
            ...
            if(~isnan(gofs))
                ...
                h_leg=annotation('textbox', [0.75 0.2 0.12 0.45],'BackgroundColor',[1 1 1],...
                    'String',{code{1},['L = ' num2str(L)],['mu = ' num2str(mu)],['kap = ' num2str(kap)],['lc = ' num2str(lc)],...
                    ['del = ' num2str(del)],['r = ' num2str(r)] ['xi = ' num2str(xi)],['xi0 = ' num2str(xi0)],['kr = ' num2str(kr)],['kr0 = ' num2str(kr0)],...
                    ['G = ' num2str(G)],['G0 = ' num2str(G0)],['end time = ' num2str(t(end))]});
...
% stoconf(minds,:)=[];
% stogofs(minds,:)=[];
save('fitvals_clean','stoG','stoG0','stoxi','stoxi0','stokr','stokr0','stoall','stoconf','stogofs');
